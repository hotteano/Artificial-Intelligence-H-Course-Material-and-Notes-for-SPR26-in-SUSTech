...
class IEMPEvaluator:
    ...
    def evaluate(self, individual: Individual, n_simulations: int = 500) -> float:
        """
        评估个体的适应度
    ...
    def fast_estimate(self, individual: Individual) -> float:
        ...
        用于进化过程中的中间评估
        """
        return self.evaluate(individual, n_simulations=200)
...
class IEMPGeneticAlgorithm:
    ...
    def evolve(self) -> Individual:
        ...
        for generation in range(self.generations):
            ...
            # 早停检查（可选，通过参数控制）
            max_no_improvement = getattr(self, 'max_no_improvement', 20)
            if max_no_improvement > 0 and no_improvement_count >= max_no_improvement:
                print(f"\nEarly stopping: No improvement for {max_no_improvement} generations")
                break
...
def main():
    ...
    parser.add_argument("--max-no-improvement", type=int, default=20, help="Early stop after N gens without improvement (0=disable, default: 20)")
    ...
    ga.max_no_improvement = args.max_no_improvement  # 设置早停参数
