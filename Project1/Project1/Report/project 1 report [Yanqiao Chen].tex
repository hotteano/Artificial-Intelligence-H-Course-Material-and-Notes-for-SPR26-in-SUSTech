...
map6 & 1142.26 & 3444.0000 & 83.5511 & 2498.824000 \\
map7 & --- & --- & 91.2650 & 2366.427800 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Heuristic Algorithm}: The \textbf{new machine (i7-14650HX) is consistently faster} by \textbf{14--58\%} across all test cases after removing console I/O bottlenecks. The high-performance Raptor Lake architecture with 16 cores demonstrates superior throughput when not blocked by synchronous print operations. The most significant speedup appears in denser graphs (map4: 58\% faster, map5: 41\% faster).
...
\textbf{Analysis}: The dramatic performance improvement on the new machine (i7-14650HX) after optimizing console output reveals that \textbf{synchronous I/O was the primary bottleneck} in the original implementation:
\begin{itemize}
    ...
    \item \textbf{Heuristic algorithm impact}: The original heuristic showed 2--3x slowdown on the new machine due to more intensive progress reporting; removing these prints inverted the comparison to 1.4--2.4x speedup
    \item \textbf{Evolutionary algorithm impact}: Similarly, removing generation progress callbacks improved new machine performance from 1.2--1.5x slower to 1.3--1.6x faster
