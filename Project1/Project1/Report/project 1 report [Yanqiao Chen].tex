...
\textbf{Note on map6 stress test.} While the heuristic pipeline was primarily designed for the provided cases (map1--map5), we conducted a separate stress test on map6 using the heuristic method to validate performance under extreme conditions. Map6 features a dense graph (3454 nodes, 32140 edges, avg\_degree=18.6) with propagation probability 1.0, representing a worst-case scenario where diffusion spreads almost completely through the network.
...
\midrule
map1 & 19.8258 & 17.1268 & \textbf{0.86x (14\% faster)} \\
map2 & 36.5812 & 21.7813 & \textbf{0.60x (40\% faster)} \\
map3 & 58.2317 & 49.2781 & \textbf{0.85x (15\% faster)} \\
map4 & 146.0283 & 61.2924 & \textbf{0.42x (58\% faster)} \\
map5 & 74.9754 & 44.2220 & \textbf{0.59x (41\% faster)} \\
\bottomrule
\end{tabular}
\end{table}
...
Case & Old Machine (s) & New Machine (s) & Ratio (New/Old) \\
\midrule
map1 & 183.3546 & 126.1788 & \textbf{0.69x (31\% faster)} \\
map2 & 121.1926 & 92.4127 & \textbf{0.76x (24\% faster)} \\
map3 & 199.9545 & 138.3765 & \textbf{0.69x (31\% faster)} \\
map4 & 28.6662 & 18.3933 & \textbf{0.64x (36\% faster)} \\
map5 & 30.7998 & 20.1344 & \textbf{0.65x (35\% faster)} \\
map6 & 83.5511 & 56.9836 & \textbf{0.68x (32\% faster)} \\
map7 & 91.2650 & 64.2292 & \textbf{0.70x (30\% faster)} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
map1 & 2.5328 & 423.938800 & 1.95x faster (Old: 4.94s) \\
map2 & 3.5323 & 35562.343000 & 1.96x faster (Old: 6.91s) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Heuristic Algorithm}: The \textbf{new machine (i7-14650HX) is consistently faster} by \textbf{7--15\%} across all test cases after removing console I/O bottlenecks. The high-performance Raptor Lake architecture with 16 cores demonstrates superior throughput when not blocked by synchronous print operations. The speedup is relatively modest for the heuristic method because its incremental diffusion computation is memory-intensive rather than compute-intensive, limiting the benefit from additional cores.

    \item \textbf{Evolutionary Algorithm}: The \textbf{new machine is faster} for all cases (map1--map7), with speedups of 24--36\%, confirming that the i7-14650HX's higher core count and clock speeds benefit the population-based evolutionary search once I/O overhead is eliminated.

    \item \textbf{Evaluator (Monte Carlo)}: The new machine is \textbf{1.95--1.96x faster} for pure evaluation tasks, nearly doubling the throughput compared to the old machine.
...
\textbf{Analysis}: The performance improvement on the new machine (i7-14650HX) after optimizing console output reveals that \textbf{synchronous I/O was the primary bottleneck} in the original implementation:
\begin{itemize}
    \item \textbf{Windows console I/O}: Frequent \texttt{print} statements in Python trigger blocking system calls and terminal refreshes, which are particularly expensive on Windows
    \item \textbf{Heuristic algorithm impact}: Removing progress reporting prints enables the new machine to achieve 7--15\% speedup over the old machine, though the gain is modest due to the memory-intensive nature of incremental diffusion
    \item \textbf{Evolutionary algorithm impact}: Removing generation progress callbacks enables the new machine to achieve 24--36\% speedup, as the population-based search benefits more from increased core count
    \item \textbf{Core utilization}: With I/O removed, the i7-14650HX can better utilize its 16 cores for parallel MC simulations and population evaluation
\end{itemize}

\textbf{Conclusion}: For compute-intensive iterative algorithms, minimizing console I/O is crucial for performance, especially on Windows systems. The i7-14650HX's superior hardware specifications (higher core count, clock speeds) become apparent only when software bottlenecks are eliminated. For reproducibility, we recommend documenting both hardware and complete software environment, as runtime performance depends heavily on implementation details like logging frequency.
