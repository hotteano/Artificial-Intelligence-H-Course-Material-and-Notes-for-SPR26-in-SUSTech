...
def evaluate_one(dim=96, epoch_num=20):
...
def main():
    epochs = [10, 15, 20, 25, 30, 35, 40, 50]
    print(f"Tuning epoch number (dim=96, baseline epoch=20):")
    print(f"{'epoch':>5} | {'AUC':>8} | {'nDCG@5':>8} | {'time(s)':>8}")
    print("-" * 42)
    results = []
    for ep in epochs:
        auc, ndcg5, t = evaluate_one(dim=96, epoch_num=ep)
        results.append((ep, auc, ndcg5, t))
        print(f"{ep:>5} | {auc:>8.5f} | {ndcg5:>8.5f} | {t:>8.1f}")

    best_auc = max(results, key=lambda x: x[1])
    best_ndcg = max(results, key=lambda x: x[2])
    print(f"\nBest AUC   : epoch={best_auc[0]}, AUC={best_auc[1]:.5f}")
    print(f"Best nDCG@5: epoch={best_ndcg[0]}, nDCG@5={best_ndcg[2]:.5f}")
