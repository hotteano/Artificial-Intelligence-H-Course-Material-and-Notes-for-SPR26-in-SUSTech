...
For CTR prediction, the submitted system first extracts five candidate scores:
\begin{equation}
    \mathbf{x}_{u,i}
    =
    \left[
    ...
    \right].
\end{equation}
Instead of using user-ID-dependent features such as the raw user index or a cold-user indicator, the final CTR branch learns a small logistic fusion model on an internal holdout split of the training data. Let $\boldsymbol{\mu}$ and $\boldsymbol{\sigma}$ be the feature mean and standard deviation on this holdout set. The CTR score is
\begin{equation}
    f_{\mathrm{CTR}}(u,i)
    =
    b
    +
    \mathbf{w}^{\top}
    \frac{\mathbf{x}_{u,i}-\boldsymbol{\mu}}{\boldsymbol{\sigma}+\epsilon}.
\end{equation}
The fusion weights $\mathbf{w}$ and bias $b$ are optimized by binary cross-entropy with a small $\ell_2$ penalty. If the learned fusion does not improve holdout AUC over the default robust mixture, the implementation falls back to the default mixture.
...
\State Learn CTR fusion weights on an internal holdout split
\For{each CTR query $(u,i)$}
    \State Extract $\mathbf{x}_{u,i}$ and return $f_{\mathrm{CTR}}(u,i)$
\EndFor
...
The final model exceeds the highest threshold for both AUC and nDCG@5. The measured runtime is also well below the project limits: initialization takes about 0.66 seconds, training including CTR fusion takes about 4.47 seconds, CTR evaluation takes about 1.22 seconds, and top-5 evaluation takes about 4.38 seconds.

\begin{table}[h]
\centering
\caption{Effect of matrix factorization training epochs before learned CTR fusion}
\begin{tabular}{rrrr}
\toprule
...
Before adding the learned CTR fusion layer, increasing the number of epochs improves AUC steadily, but nDCG@5 peaks at 20 epochs and slightly decreases afterwards. This suggests that longer training improves pointwise discrimination but may start to overfit the ranking objective. Therefore, 20 epochs are used for matrix factorization, and the final CTR calibration is then learned on an internal holdout split.

\subsection{External MovieLens-1M Sanity Check}

To test whether the model behavior is reasonable beyond the released project arrays, an additional sanity check is conducted on the original MovieLens-1M files. Ratings greater than or equal to 4 are treated as positive implicit feedback. For the implicit setting, negative samples are drawn from movies that the corresponding user has not rated. Movie genre, year, and decade are converted into simple KG triplets. This external test is not part of the official grading protocol, but it helps verify that the model is not only exploiting the specific project split.

\begin{table}[h]
\centering
\caption{External MovieLens-1M validation}
\begin{tabular}{lrr}
\toprule
Negative sampling protocol & AUC & nDCG@5 \\
\midrule
Low explicit ratings as negatives & 0.8917 & 0.2348 \\
Unrated movies as negatives & 0.9388 & 0.2798 \\
\bottomrule
\end{tabular}
\end{table}

The model obtains high AUC and nDCG@5 under both protocols. The unrated-negative protocol is closer to an implicit-feedback recommendation scenario and gives stronger performance, indicating that the hybrid signals generalize to a standard recommendation benchmark.
...
The current implementation also has limitations. First, the CTR fusion layer is learned only as a shallow logistic model rather than as an end-to-end neural architecture. Second, the KG component is based on sparse relation-neighbor features instead of deep relational message passing. Third, the local temporal prior uses user ID order as an empirical signal, which is effective for the provided data but may not generalize if the test distribution is randomly permuted. To reduce this risk in CTR prediction, the final system avoids raw user-ID and cold-user indicator features and relies on learned fusion of interaction, KG, CF, temporal, and popularity scores.

\section{Conclusion}

This project implements a hybrid KG-based recommender system for CTR prediction and top-5 recommendation. The final algorithm integrates popularity, temporal locality, KG content profiles, item-based collaborative filtering, BCE+BPR-trained matrix factorization, and a learned logistic CTR fusion layer. A user-aware 80/20 validation protocol is adopted to avoid artificial cold-start bias caused by sorted interaction arrays. The final system obtains an AUC of 0.8570 and an nDCG@5 of 0.1456, satisfying the highest score thresholds under the local validation setting while remaining computationally efficient on CPU.
